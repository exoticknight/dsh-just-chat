...
- 两个入口生成相同名称时复用同一工作区并各自创建原生会话；名称不同则使用不同工作区。原生“新建会话”仍使用所在工作区的目录。
- 归档会话、删除工作区后，再次点击入口仍可对话；刷新后记录正常保留。
- 插件详情页的两个入口开关、模板预览与校验、保存、离开页面时丢弃草稿、语言切换，以及刷新和重启后的配置保留。
- 窄窗口布局、附件、取消响应和断线恢复。
...
| `client.js` | 入口、原生 UI 组件、插件详情页配置表单和命名模板 |
| `cordis.patch.yml` | DSH bundle 的 Host 激活配置 |
| `scripts/dsh-dev.js` | 安装项目内 DSH 与调试插件，或启动隔离的调试环境 |
| `tests/` | 入口调用与命名规则测试 |

所有功能在插件中实现，不修改 DSH 宿主源码。首页入口使用 `conversation.hero.agentPreset` 的 DOM 锚点，侧栏入口和插件详情页配置使用官方插槽；升级 DSH 时需重新验证入口挂载和布局。
